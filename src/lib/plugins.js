...
const { theme, chalk, BOX } = require('./colors');
...
/**
 * Formats and prints the loaded plugins in a table
 * @param {Array<{name: string, category: string}>} plugins 
 */
function printPluginTable(plugins) {
    ...
    plugins.sort((a, b) => {
        const catA = safeStr(a.category);
        const catB = safeStr(b.category);
        const nameA = safeStr(a.name);
        const nameB = safeStr(b.name);
        
        if (catA === catB) return nameA.localeCompare(nameB);
    ...
    const table = new Table({
        head: [theme.primary('Category'), theme.primary('Plugins Loaded')],
        colWidths: [20, 60],
        wordWrap: true,
        style: {
            head: [],
    ...
    for (const [category, names] of Object.entries(grouped)) {
        // Chunk names into groups of 4 for better display
        const chunks = [];
        const chunkSize = 4;
        ...
        // Add to table
        table.push([
            { rowSpan: chunks.length, content: theme.secondary(category.toUpperCase()), vAlign: 'center' },
            theme.text(chunks[0])
        ]);
        
        // Add remaining chunks
        for (let i = 1; i < chunks.length; i++) {
            table.push([theme.text(chunks[i])]);
        }
    }

    console.log('');
    console.log(table.toString());
    console.log(`${theme.border('║')} Total: ${theme.primary(plugins.length + ' Plugins')}`);
