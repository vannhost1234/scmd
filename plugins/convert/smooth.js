...
const pluginConfig = {
    ...
    alias: ['mellow', 'soft'],
    category: 'convert',
    description: 'Menambahkan efek smooth/mellow ke audio',
    usage: '.smooth (reply audio/video)',
    example: '.smooth',
    isOwner: false,
    isPremium: false,
    isGroup: false,
...
async function handler(m, { sock }) {
    ...
    const outputPath = path.join(tempDir, `smooth_${Date.now()}.mp3`)
    
    try {
        const buffer = await downloadFn()
        fs.writeFileSync(inputPath, buffer)
        
        execSync(`ffmpeg -y -i "${inputPath}" -af "lowpass=f=4000,bass=g=3:f=100,treble=g=-2:f=3000,aecho=0.8:0.88:60:0.4" -vn "${outputPath}"`, { stdio: 'ignore' })
