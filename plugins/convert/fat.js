...
const pluginConfig = {
    ...
    alias: ['fatbass', 'thicc'],
    category: 'convert',
    description: 'Menambahkan efek fat/thick bass ke audio',
    usage: '.fat (reply audio/video)',
    example: '.fat',
    isOwner: false,
    isPremium: false,
    isGroup: false,
...
async function handler(m, { sock }) {
    ...
    const outputPath = path.join(tempDir, `fat_${Date.now()}.mp3`)
    
    try {
        const buffer = await downloadFn()
        fs.writeFileSync(inputPath, buffer)
        
        execSync(`ffmpeg -y -i "${inputPath}" -af "bass=g=15:f=60:w=0.8,lowpass=f=3000,volume=1.5" -vn "${outputPath}"`, { stdio: 'ignore' })
